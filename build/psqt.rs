...
use crate::{i32_list, list};

type Psqt = [i32; 64];

pub fn build() -> String {
    format!(
        "static PSQT_NON_KING: [[i32; 64]; 12] = {};\n\
         static PSQT_MG_KING: [[i32; 64]; 2] = {};\n\
         static PSQT_EG_KING: [[i32; 64]; 2] = {};\n",
        list(psqt_non_king().iter().map(|values| i32_list(values))),
        list(psqt_king(&PSQT_MG_WHITE_KING).iter().map(|values| i32_list(values))),
        list(psqt_king(&PSQT_EG_WHITE_KING).iter().map(|values| i32_list(values))),
    )
}

fn psqt_non_king() -> [Psqt; 12] {
...
fn psqt_king(psqt_white_king: &Psqt) -> [Psqt; 2] {
    let mut psqt = [[0; 64]; 2];
